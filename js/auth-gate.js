...
// SHA-256 en JS puro, sin dependencias. Se usa como respaldo cuando el
// navegador bloquea Web Crypto (crypto.subtle solo funciona en "contextos
// seguros": HTTPS o localhost). Da exactamente el mismo resultado que
// crypto.subtle.digest, así que un único AUTH_HASH_DEFAULT sirve para los
// dos casos.
function sha256Puro(mensaje) {
  const K = [
...
async function hashClave(texto) {
  ...
  // Contexto no seguro (HTTP sin certificado todavía, por ejemplo): se usa
  // el cálculo en JS puro, que da el mismo resultado.
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  form.addEventListener('submit', async (ev) => {
    ...
    try {
      ...
      err.textContent = 'No se pudo verificar la clave (error técnico). Probá recargar la página; si persiste, avisá al desarrollador.';
    }
  });
